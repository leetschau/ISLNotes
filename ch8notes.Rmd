...
对于线性问题，树方法的解释性（什么叫模型的可解释性？解释性与模型的哪一个性质一般要做 trade-off？）非常好，但不如增强型线性回归预测精度高，例如第6章的 Ridge regression, LASSO，第7章的多项式和样条回归，（各自的思想是什么？），为了提高预测精度，在基本树方法的基础上又发展出 bagging ,random forest, boosting，以牺牲解释性（和增加计算成本）为代价，提升预测精度。
...
第一次分割后，选择两个分区中的一个（选择标准是什么？）重复上述过程，直到满足某个停止准则，例如每个分区内的观测点数都小于5。
...
对于某一个观测，一次 bootstrap 抽样大约覆盖 2/3 的样本（参考文献1），所以一次抽样中作为测试数据（未被抽中）的概率是 1/3，
