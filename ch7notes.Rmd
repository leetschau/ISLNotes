...
光滑曲线方法的计算公式 (7.11) 与 第6章 ridge regression 和 lasso 的思想是类似的，都是在 training RSS 的基础上增加一个带调节参数 $\lambda$ 的 penalty 项，从而避免过拟合。
区别在于后两者通过控制系数的1阶 (lasso) 和2阶 (ridge) 模总和（书中用“预算” budget 来比喻十分贴切）达到目的，而 smooth spline 通过控制曲线粗糙度 (roughness) 达到目的。
